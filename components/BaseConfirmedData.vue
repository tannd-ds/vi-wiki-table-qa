    ...
    <TabGroup as="div" class="flex gap-4">
      ...
      <BaseBox :override_bg="true" class="p-4">
        <TabPanels as="div" class="w-[35vw] min-w-[35vw] flex flex-1 flex-col gap-4">
          <TabList
              as="div"
              class="flex space-x-1 rounded-xl bg-gray-300 p-1 dark:bg-zinc-700/20"
          >
            <Tab
            ...
            >
              ...
              >
                <UIcon class="text-2xl" :name="tabs[tab].icon" />
            ...
            <div class="flex items-center justify-between gap-8">
              ...
              >
                <TransitionGroup name="fade">
                  <li
                    class="mr-4 py-3 flex flex-col gap-2 hover:cursor-pointer sm:py-4"
                    v-for="(qa, qa_index) in filter_confirmed"
                    :key="qa_index"
                    @click.prevent="set_chosen_qa(qa_index)"
                  >
                    <div class="flex items-center gap-2">
                      <UIcon name="i-heroicons-question-mark-circle" class="text-md" />
                      <span
                        class="text-sm font-medium text-midnight-100 dark:text-gray-200"
                        :class="{
                            truncate: qa_index !== state.current_qa_index,
                            'text-midnight-200 dark:text-green-300': qa_index === state.current_qa_index,
                          }"
                      >
                        {{ qa.question }}
                      </span>
                    ...
                    <div class="flex items-center gap-2">
                      <UIcon name="i-heroicons-chat-bubble-bottom-center" class="text-md" />
                      <span
